import cover from "@/assets/bookCovers/cover.jpeg";
import coverJames from "@/assets/bookCovers/coverJames.jpg";

const articlesArray = [
  {
    id: 1,
    name: "أساسيات البرمجة",
    synopsis: "دليل شامل للمبتدئين لفهم أساسيات البرمجة وتعلم المفاهيم الأساسية",
    body: "البرمجة هي عملية كتابة تعليمات وتوجيهات لجهاز الكمبيوتر لتنفيذ مهام محددة. تتكون أساسيات البرمجة من المتغيرات، والشروط، والحلقات التكرارية، والدوال. فهم هذه المفاهيم يساعدك على بناء تطبيقات بسيطة ومعقدة.",
    author: "د. أحمد الخليلي",
    date: "2024-01-15",
  },
  {
    id: 2,
    name: "تاريخ الحضارة الإسلامية",
    synopsis: "نظرة على الإنجازات العلمية والثقافية في العصر الذهبي للإسلام",
    body: "شهدت الحضارة الإسلامية في العصور الوسطى ازدهاراً كبيراً في مجالات العلوم والفلسفة والفنون. برز علماء مثل ابن سينا والخوارزمي وابن رشد، الذين ساهموا في تطور المعرفة الإنسانية وأسسوا للنهضة الأوروبية.",
    author: "أ.د. سعاد الصباح",
    date: "2024-02-20",
  },
  {
    id: 3,
    name: "فن الطهي العربي",
    synopsis: "استكشاف تقاليد وأطباق المطبخ العربي الأصيل",
    body: "يتميز المطبخ العربي بتنوعه حسب المناطق، ويشمل أطباقاً شهيرة مثل المنسف، والكسكس، والكبسة. تعتمد الوصفات العربية على البهارات المميزة والأعشاب الطازجة، وتعكس كرم الضيافة العربية الأصيلة.",
    author: "الشيف منى الموسى",
    date: "2024-03-05",
  },
  {
    id: 4,
    name: "العمارة الإسلامية",
    synopsis: "جماليات وتفاصيل العمارة في المساجد والقصور الإسلامية",
    body: "تتميز العمارة الإسلامية باستخدام الزخارف الهندسية والنباتية، والقباب، والمآذن. من أبرز الأمثلة على ذلك قصر الحمراء في الأندلس، والمسجد الأموي في دمشق، ومسجد السلطان أحمد في اسطنبول.",
    author: "د. بدر العمران",
    date: "2024-01-28",
  },
  {
    id: 5,
    name: "الشعر العربي الحديث",
    synopsis: "تطور القصيدة العربية من الكلاسيكية إلى الحداثة",
    body: "مر الشعر العربي بمراحل تطور متعددة، من القصيدة العمودية إلى قصيدة النثر. برزت مدارس شعرية مثل مدرسة الشعر الحر مع بدر شاكر السياب، ومدرسة الشعر الحديث مع أدونيس ومحمود درويش.",
    author: "د. نادية المصري",
    date: "2024-02-12",
  },
  {
    id: 6,
    name: "القهوة العربية",
    synopsis: "ثقافة وتقاليد إعداد وتقديم القهوة في الوطن العربي",
    body: "تعتبر القهوة العربية رمزاً للكرم والترحاب في المجتمعات العربية. تختلف طرق إعدادها من منطقة لأخرى، من القهوة السعودية بالهيل والزعفران إلى القهوة البدوية السادة. تقدم عادة في دلة خاصة مع التمر.",
    author: "عبدالله السعدي",
    date: "2024-03-18",
  },
  {
    id: 7,
    name: "الخط العربي",
    synopsis: "فن وتاريخ الخطوط العربية وأنواعها المختلفة",
    body: "يعد الخط العربي من أرقى الفنون الإسلامية، وتطورت منه عدة مدارس وأنماط مثل الخط الكوفي، والنسخ، والثلث، والديواني. يستخدم الخط العربي في تزيين المساجد والمصاحف واللوحات الفنية.",
    author: "خالد البغدادي",
    date: "2024-02-25",
  },
];
const booksArray = [
  {
    id: 0,
    name: "والدي",
    synopsis:
      "فقد عرفت الشيخ ' علي بن عبد الله الحصين ' شابا ملتزما ، يتمتع بخلق قويم، ويسير في حياته على نهج صائب، يتمسك فيه بالمبادئ الصحيحة، والمثل العليا .. أحبه من عرفه ، فقد تلمسوا في شخصيته المساعد والمعين في مواجهة شدائد الحياة وخطوب الزمن، إذ نذر نفسه وحياته لخدمة الآخرين، وفعل الخير. كان نموذجا صادقا للصلاح والتقوى، والوعي والثقافة الشاملة التي كانت دافعة لأن",
    cover: cover,
    author: "",
    date: "2009",
  },

  {
    id: 2,
    name: "رجل من زمن النسيان",
    synopsis:
      "رجل مسن يعيش في دار للمسنين يدّعي أنه كان طيارًا في حرب أكتوبر، لكن لا أحد يصدقه. عندما تزور شابة صيدلانية الدار ضمن العمل التطوعي، تجد مذكراته القديمة وصورًا faded لطائرات حربية. تبدأ رحلة البحث عن حقيقته لتكتشف أنه كان بطلاً نساه الجميع.",
    cover: coverJames,
    author: "محمد العبدالله",
    date: "2024-02-14",
  },
  {
    id: 3,
    name: "دمشق تحترق في عيني",
    synopsis:
      "رسامة سورية تهاجر إلى باريس بعد اندلاع الحرب، تحاول بناء حياة جديدة لكن ذكريات وطنها تلاحقها. تعود إلى دمشق بعد عشر سنوات لتبحث عن لوحة رسمتها طفلة مجهولة قبل الحرب، وتكتشف أن اللوحة أصبحت رمزًا للمقاومة والسلام.",
    cover: "",
    author: "سمر يزبك",
    date: "2022-11-08",
  },
  {
    id: 4,
    name: "سر الجرة التاسعة",
    synopsis:
      "في متحف الآثار بالقاهرة، تكتشف مفتشة الآثار ياسمين جرة فخارية غامضة ضمن مجموعة غير موثقة. عندما تفتحها، تجد بداخلها خريطة غريبة ومفاتيح نحاسية قديمة. تبدأ مطاردة مع تجار الآثار غير الشرعيين للوصول إلى كنز فرعوني حقيقي قبل أن يهرب خارج البلاد.",
    cover: "",
    author: "أحمد مراد",
    date: "2023-09-30",
  },
  {
    id: 5,
    name: "أولاد القصبة",
    synopsis:
      "خمسة أطفال نشأوا في قصبة عمان القديمة في الثمانينات، يلعبون معًا ويكبرون معًا. تتابع الرواية مسارات حياتهم المختلفة بعد أربعين عامًا: من أصبح تاجرًا، ومن هاجر، ومن بقي في المكان نفسه. تلتقي بهم الأقدار مجددًا عندما يهدد مشروع استثماري كبير بهدم حيهم القديم.",
    cover: "",
    author: "هند الفايز",
    date: "2024-01-05",
  },
];

export { articlesArray, booksArray };
